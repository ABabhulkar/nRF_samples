- Create an HTTP request of type [`struct http_request`](https://developer.nordicsemi.com/nRF_Connect_SDK/doc/latest/zephyr/connectivity/networking/api/http.html#c.http_request), which will contain all the data necessary for doing an HTTP request.
- When populating the HTTP request, we provided a callback function to be called when a response to the HTTP request is received.
- When the HTTP request is populated, we will send it using the API call [http_client_req()](https://docs.nordicsemi.com/bundle/ncs-latest/page/zephyr/connectivity/networking/api/http_client.html#c.http_client_req).
> [!note]
...
##### Steps to configure http client
- Enable the client lib ``CONFIG_HTTP_CLIENT=y``
- Include the header file ``#include <zephyr/net/http/client.h>``
- example code
```c
...
  int err = 0;
  const char *headers[] = {"connection closed\r\n", NULL};
...
  err = http_client_req(sock, &req, 5000, NULL);
  if (err < 0) {
    LOG_ERR("Faild to send HTTP post request, err:%d", err);
  }
...
> [!note]
> Client ID is a random string of 36 characters.
> Make `client_id_buf` two characters longer to add a null terminator at the end and a front slash at the beginning, making it easier to pass as a URL to the HTTP client library.
...
# Enable Zephyr's HTTP parser functionality
